...
2. Hashlab loops over a nested directory with vagrantfiles
3. Each and every vagrantfile is executed
4. After running the vagrant box the virtual hard drive is cloned as raw image 
...
Currently the list of MD5-hashes are formed as ~md5sum~ would do it, to ensure an easy ingestion in industry standard tools like Autopsy or X-Ways. They look as follows:
#+BEGIN_SRC 
